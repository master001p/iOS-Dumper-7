...
public:
	inline AllFieldIterator begin() const { return AllFieldIterator(); }
	inline AllFieldIterator end() const   { return AllFieldIterator(ObjectArray().end()); }
...
private:
	...
	inline UEStruct GetCurrentStruct() { return (*CurrentObject).Cast<UEStruct>(); }
	inline bool IsCurrentObjectStruct() { return (*CurrentObject).IsA(EClassCastFlags::Struct); }
	inline bool IsEndIterator() const { return !(CurrentObject != ObjectEndIterator); }
};
